...
import { Literal, Link, Values, DataObject } from "./value";
...
export class PageMetadata {
    ...
    public constructor(path: string, init?: Partial<PageMetadata>) {
        ...
        this.lists = ([]).map(l => new ListItem(l));
    }
    ...
    public static canonicalize(data: Partial<PageMetadata>, linkNormalizer: (link: Link) => Link): PageMetadata {
        ...
        if (data.frontmatter) {
            data.frontmatter = Values.mapLeaves(data.frontmatter, (t: any) =>
        ...
        if (data.fields) {
            for (let [key, value] of data.fields.entries()) {
                data.fields.set(
                    key,
                    Values.mapLeaves(value, (t: any) => (Values.isLink(t) ? linkNormalizer(t) : t))
                );
            }
        }
        ...
        if (data.lists) {
            for (let item of data.lists) {
                for (let [key, value] of item.fields.entries()) {
                    item.fields.set(
                        key,
                        value.map(x => Values.mapLeaves(x, (t: any) => (Values.isLink(t) ? linkNormalizer(t) : t)))
                    );
                }
            }
        }
